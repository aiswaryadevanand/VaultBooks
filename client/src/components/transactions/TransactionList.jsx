...
//     const matchesDate =
//       !filters.date || tx.date?.substring(0, 10) === filters.date;
//     const matchesTag =
//       !filters.tags ||
//       (tx.tags &&
//         tx.tags.some((tag) =>
...
//         ⚠️ Please select a wallet to view transactions.
//       </p>
//     );
//   if (isLoading) return <p>Loading...</p>;
//   if (isError)
...
//                       {tx.walletId?.name || 'From'} →{' '}
//                       {tx.toWalletId?.name || 'To'}
//                     </td>
//                   ) : (
            ...
            {filteredData.map((tx) => (
              <tr key={tx._id} className="border-t hover:bg-gray-50">
                ...
                {hasTransfer && (
                  <td className="py-2 px-3 text-sm text-gray-700">
                    {tx.type === 'transfer'
                      ? `${tx.isMirror ? tx.toWalletId?.name : tx.walletId?.name} → ${tx.isMirror ? tx.walletId?.name : tx.toWalletId?.name}`
                      : '—'}
                  </td>
                )}
                <td className="py-2 px-3">{new Date(tx.date).toLocaleDateString()}</td>
